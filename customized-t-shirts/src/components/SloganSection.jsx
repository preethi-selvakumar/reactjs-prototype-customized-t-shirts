...
import { sloganProducts } from '../data/sloganProducts';
...
const SloganSection = () => {
    ...
    const [productsToDisplay, setProductsToDisplay] = useState(sloganProducts);
    ...
    useEffect(() => {
        ...
        if (searchTerm.trim() !== '') {
            // If there is a search term, filter and display sloganProducts
            const searchedProducts = sloganProducts.filter(product =>
            ...
            // If there's no search term, show sloganProducts again
            setProductsToDisplay(sloganProducts);
        }
    }, [searchTerm]);
    ...
    const secondBatch = productsToDisplay.slice(8, 12);
        ...
        <>
            ...
            {productsToDisplay.length > 0 ? (
                ...
                <div className="no-products-message">
                    <p>
                        {searchTerm.trim() !== ''
                            ? "No Slogan products found matching your search."
                            : "No Slogan products available."}
                    </p>
                </div>
            )}
        </>
